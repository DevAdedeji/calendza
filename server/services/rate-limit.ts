...
export async function enforceRateLimit(event: H3Event, options: RateLimitOptions) {
  ...
  const bucket = await consumeRateLimit(source, options.limit, options.windowSeconds)
  const resetAt = Math.ceil(bucket.expiresAt.getTime() / 1000)
  setResponseHeader(event, 'RateLimit-Limit', String(options.limit))
  setResponseHeader(event, 'RateLimit-Remaining', String(bucket.remaining))
...
export async function consumeRateLimit(source: string, limit: number, windowSeconds: number) {
  ...
  const [bucket] = await useDatabase()
    ...
    .values({
      ...
      windowStart: sql`now()`,
      requestCount: 1,
      expiresAt: sql`now() + (${windowSeconds} * interval '1 second')`
    })
    .onConflictDoUpdate({
      ...
      set: {
        ...
        expiresAt: sql`case when ${apiRateLimits.expiresAt} <= now() then now() + (${windowSeconds} * interval '1 second') else ${apiRateLimits.expiresAt} end`
      }
    })
