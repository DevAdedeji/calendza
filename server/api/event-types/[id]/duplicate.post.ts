...
export default defineEventHandler(async (event) => {
  ...
  const [created] = await db.insert(eventTypes).values({
    ...
    additionalDurationMinutes: source.additionalDurationMinutes,
    recurringBookingEnabled: source.recurringBookingEnabled,
    recurringBookingMaxOccurrences: source.recurringBookingMaxOccurrences,
