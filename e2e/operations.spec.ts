...
test('keeps operations private and retries a failed delivery job', async ({ page, request }) => {
  ...
  const emailQueue = page.getByRole('tab', { name: 'Email delivery' })
  await emailQueue.click()
  await expect(emailQueue).toHaveAttribute('aria-selected', 'true')
  const failedFilter = page.getByRole('tab', { name: 'Failed', exact: true })
