...
test('creates a team, invites an existing admin and completes a shared booking', async ({ page }) => {
  ...
  await signUpAndSignIn(page, {
    ...
    email: 'grace-team-host@calendza.test'
  })
  ...
  await signUpAndSignIn(page, {
    ...
    email: 'ada-team-owner@calendza.test'
  })
  await createTeam(page, 'Quality Labs', 'quality-labs')

  const invitationId = await invite(page, 'grace-team-host@calendza.test', 'admin')
  await signOutLocally(page)
  await signIn(page, 'grace-team-host@calendza.test', `/invite/${invitationId}`)
  ...
  await page.getByLabel('Email').fill('workspace-guest@calendza.test')
  ...
  const [booking] = await sql<{ organizationId: string, hostCount: number }[]>`
    ...
    where b.attendee_email = 'workspace-guest@calendza.test'
    group by b.id
  `
...
test('guides an invited new user through account creation without exposing owner controls', async ({ page }) => {
  await signUpAndSignIn(page, {
    ...
    email: 'workspace-owner@calendza.test'
  })
  await createTeam(page, 'Newcomer Studio', 'newcomer-studio')
  const invitationId = await invite(page, 'new-member@calendza.test')

  await page.goto(`/invite/${invitationId}`)
  await expect(page.getByText(/this invitation is for new-member@calendza\.test/i)).toBeVisible()
  ...
  await expect(page.getByTestId('signup-form')).toHaveAttribute('data-ready', 'true')
  await expect(page.getByLabel('Email')).toHaveValue('new-member@calendza.test')
  ...
  await sql`update users set email_verified = true where email = 'new-member@calendza.test'`
  await signIn(page, 'new-member@calendza.test', `/invite/${invitationId}`)
  await page.getByRole('button', { name: 'Accept invitation' }).click()
  ...
  const [membership] = await sql<{ role: string }[]>`
    ...
    where u.email = 'new-member@calendza.test' and o.slug = 'newcomer-studio'
  `
...
test('manages synchronized member links and scopes team analytics exports', async ({ page }) => {
  ...
  const memberEmail = 'managed-member@calendza.test'
  const ownerEmail = 'managed-owner@calendza.test'
  ...
  await sql`
    insert into bookings (
      ...
      (${organizationRow!.id}, ${managed!.eventTypeId}, ${memberUser!.id}, ${crypto.randomUUID()}, 'confirmed', now() + interval '1 day', now() + interval '1 day 45 minutes', 'Member Guest', 'member-guest@calendza.test', 'UTC', 'hosted', now()),
      (${organizationRow!.id}, ${managed!.sourceEventTypeId}, ${ownerUser!.id}, ${crypto.randomUUID()}, 'confirmed', now() + interval '2 days', now() + interval '2 days 45 minutes', 'Owner Guest', 'owner-guest@calendza.test', 'UTC', 'hosted', now())
  `
  ...
  expect(memberCsv).toContain('member-guest@calendza.test')
  expect(memberCsv).not.toContain('owner-guest@calendza.test')
  ...
  expect(ownerCsv).toContain('member-guest@calendza.test')
  expect(ownerCsv).toContain('owner-guest@calendza.test')
...
test('revokes invitations, transfers ownership and archives the team safely', async ({ page }) => {
  ...
  await signUpAndSignIn(page, {
    ...
    email: 'future-owner@calendza.test'
  })
  await signOutLocally(page)
  ...
  await signUpAndSignIn(page, {
    ...
    email: 'original-owner@calendza.test'
  })
  await createTeam(page, 'Lifecycle Team', 'lifecycle-team')

  const revokedId = await invite(page, 'revoked-member@calendza.test')
  const revokedRow = page.getByRole('listitem').filter({ hasText: 'revoked-member@calendza.test' })
  await revokedRow.getByRole('button', { name: 'Revoke' }).click()
  await expect(page.getByText('revoked-member@calendza.test', { exact: true })).toHaveCount(0)
  ...
  const invitationId = await invite(page, 'future-owner@calendza.test')
  await signOutLocally(page)
  await signIn(page, 'future-owner@calendza.test', `/invite/${invitationId}`)
  await page.getByRole('button', { name: 'Accept invitation' }).click()
  await expect(page).toHaveURL(/\/t\/lifecycle-team\/members$/)

  await signOutLocally(page)
  const ownerNext = '/t/lifecycle-team/members'
  const ownerLogin = new URLSearchParams({ next: ownerNext, email: 'original-owner@calendza.test' })
  await page.goto(`/login?${ownerLogin}`)
  await page.waitForLoadState('networkidle')
  await page.getByLabel('Email').fill('original-owner@calendza.test')
  await page.locator('input[name="password"]').fill(password)
  await page.getByRole('button', { name: 'Sign in' }).click()
  ...
  await page.getByText(/Future Owner · future-owner@calendza\.test/).click()
  ...
  await signOutLocally(page)
  await signIn(page, 'future-owner@calendza.test', '/t/lifecycle-team/settings')
